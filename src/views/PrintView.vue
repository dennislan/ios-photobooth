<template>
  <div class="flex flex-col h-full">
    <div class="flex items-center justify-between px-6 py-3 bg-white border-b border-border-color">
      <h2 class="text-lg font-semibold m-0">预览与打印</h2>
      <a-button @click="$emit('back')">
        <template #icon><ArrowLeftOutlined /></template>
        返回
      </a-button>
    </div>

    <div class="flex-1 flex gap-6 p-6 overflow-hidden">
      <div class="flex-1 flex flex-col">
        <a-empty v-if="layoutPhotos.length === 0" description="请先选择照片" class="!flex !items-center !justify-center !h-full" />
        <div v-else class="flex-1 flex flex-col bg-white rounded-lg border border-border-color overflow-hidden">
          <div class="flex-1 flex items-center justify-center p-4 overflow-hidden">
            <canvas ref="canvasRef" class="max-w-full max-h-full object-contain shadow-lg rounded"></canvas>
          </div>
          <div class="p-3 border-t border-border-color flex items-center gap-3 text-sm text-text-secondary">
            <span>模板: {{ currentLayoutName }}</span><span>|</span>
            <span>滤镜: {{ filterLabel }}</span><span>|</span>
            <span>{{ layoutPhotos.length }} 张照片</span>
          </div>
        </div>
      </div>

      <a-card title="打印设置" :bordered="false" class="w-[300px] flex-shrink-0">
        <a-form layout="vertical">
          <a-form-item label="纸张大小">
            <a-select v-model:value="captureStore.paperSize">
              <a-select-option v-for="p in paperSizes" :key="p.value" :value="p.value">{{ p.label }}</a-select-option>
            </a-select>
          </a-form-item>
          <a-form-item label="色彩模式">
            <a-select v-model:value="captureStore.colorMode">
              <a-select-option v-for="c in colorModes" :key="c.value" :value="c.value">{{ c.label }}</a-select-option>
            </a-select>
          </a-form-item>
          <a-form-item label="份数">
            <a-input-number v-model:value="captureStore.copies" :min="1" :max="20" style="width: 100%;" />
          </a-form-item>
          <a-divider style="margin: 8px 0;" />
          <a-form-item label="滤镜调整">
            <a-select :value="captureStore.filter" @change="(v: any) => captureStore.setFilter(v)">
              <a-select-option v-for="f in filterKeys" :key="f" :value="f">{{ filterLabels[f] }}</a-select-option>
            </a-select>
          </a-form-item>
        </a-form>
      </a-card>
    </div>

    <div class="px-6 py-4 bg-white border-t border-border-color">
      <a-button
        type="primary" size="large" block :loading="printing"
        @click="printImage" class="!py-3.5 !text-base"
        style="background: #34C759; border-color: #34C759;"
      >
        <template #icon><PrinterOutlined /></template>
        {{ printing ? '打印中...' : '打印照片' }}
      </a-button>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ArrowLeftOutlined, PrinterOutlined } from '@ant-design/icons-vue';
import { ref, computed, onMounted, watch, nextTick } from 'vue';
import { invoke } from '@tauri-apps/api/core';
import { message } from 'ant-design-vue';
import {
  useCaptureStore, FILTER_CSS, FILTER_LABELS, LAYOUTS, PAPER_SIZES, COLOR_MODES, type FilterType,
} from '../stores/capture';

defineEmits<{ back: [] }>();

const captureStore = useCaptureStore();
const canvasRef = ref<HTMLCanvasElement | null>(null);
const printing = ref(false);

const paperSizes = PAPER_SIZES;
const colorModes = COLOR_MODES;
const filterKeys = Object.keys(FILTER_LABELS) as FilterType[];
const filterLabels = FILTER_LABELS;

const layoutPhotos = computed(() => captureStore.photosForLayout);
const filterLabel = computed(() => FILTER_LABELS[captureStore.filter]);
const currentLayoutName = computed(() => LAYOUTS.find((t) => t.id === captureStore.layoutId)?.name ?? '默认');

function canvasDimensions(): { w: number; h: number } {
  switch (captureStore.layoutId) {
    case 'single': return { w: 1200, h: 1800 };
    case 'strip': return { w: 900, h: 1800 };
    default: return { w: 1200, h: 1800 };
  }
}

onMounted(() => nextTick(renderPreview));
watch([layoutPhotos, () => captureStore.filter, () => captureStore.layoutId], () => nextTick(renderPreview));

function loadImage(src: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.crossOrigin = 'anonymous';
    img.onload = () => resolve(img);
    img.onerror = reject;
    img.src = src;
  });
}

async function renderPreview() {
  const canvas = canvasRef.value;
  if (!canvas || layoutPhotos.value.length === 0) return;
  const { w, h } = canvasDimensions();
  canvas.width = w;
  canvas.height = h;
  const ctx = canvas.getContext('2d');
  if (!ctx) return;
  ctx.fillStyle = '#FFFFFF';
  ctx.fillRect(0, 0, w, h);
  const filterCss = FILTER_CSS[captureStore.filter];
  ctx.filter = filterCss === 'none' ? 'none' : filterCss;
  const layout = LAYOUTS.find((t) => t.id === captureStore.layoutId);
  const cols = layout?.cols ?? 2;
  const rows = layout?.rows ?? 2;
  const padding = 40;
  const gap = 20;
  const cellW = (w - padding * 2 - gap * (cols - 1)) / cols;
  const cellH = (h - padding * 2 - gap * (rows - 1)) / rows;
  const images: HTMLImageElement[] = [];
  for (const photo of layoutPhotos.value) {
    try { images.push(await loadImage(photo.dataUrl)); } catch { /* skip */ }
  }
  images.forEach((img, idx) => {
    if (idx >= cols * rows) return;
    const col = idx % cols;
    const row = Math.floor(idx / cols);
    const x = padding + col * (cellW + gap);
    const y = padding + row * (cellH + gap);
    const imgRatio = img.width / img.height;
    const cellRatio = cellW / cellH;
    let dw: number, dh: number;
    if (imgRatio > cellRatio) { dh = cellH; dw = cellH * imgRatio; }
    else { dw = cellW; dh = cellW / imgRatio; }
    const dx = x + (cellW - dw) / 2;
    const dy = y + (cellH - dh) / 2;
    ctx.save();
    ctx.beginPath();
    ctx.rect(x, y, cellW, cellH);
    ctx.clip();
    ctx.drawImage(img, dx, dy, dw, dh);
    ctx.restore();
  });
  ctx.filter = 'none';
}

function canvasToBlob(quality = 0.92): Promise<Blob | null> {
  const canvas = canvasRef.value;
  if (!canvas) return Promise.resolve(null);
  return new Promise((resolve) => canvas.toBlob(resolve, 'image/jpeg', quality));
}

function blobToBase64(blob: Blob): Promise<string> {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = () => resolve(reader.result as string);
    reader.onerror = reject;
    reader.readAsDataURL(blob);
  });
}

async function printImage() {
  if (layoutPhotos.value.length === 0) { message.warning('请先选择照片'); return; }
  printing.value = true;
  try {
    const blob = await canvasToBlob(0.92);
    if (!blob) { message.error('图片生成失败'); return; }
    const base64 = await blobToBase64(blob);
    const imagePath = (await invoke('save_print_image', { data: base64, ext: 'jpg' })) as string;
    await invoke('print_photo', {
      imagePath,
      paperSize: captureStore.paperSize,
      colorMode: captureStore.colorMode,
      copies: captureStore.copies,
    });
    message.success(`打印成功！已发送 ${captureStore.copies} 份到打印机`);
  } catch (e) {
    message.error(e instanceof Error ? e.message : String(e));
  } finally {
    printing.value = false;
  }
}
</script>
